...
** Magit
   ...
   - Docs :: https://magit.vc/
     ...
     #+end_src
** mlscrollbar
   - Repo :: https://github.com/jdtsmith/mlscroll
     #+begin_src emacs-lisp
...
** Move point to other window immediately after split
   #+begin_src emacs-lisp
     (defun adb-split-below ()
       "Create a new window below and move point to new window."
       (interactive)
       (split-window-below)
       (other-window 1))

     (defun adb-split-right ()
       "Create a new window to the right and move point to new window."
       (interactive)
       (split-window-horizontally)
       (other-window 1))

     (global-set-key (kbd "C-x 2") 'adb-split-below)
     (global-set-key (kbd "C-x 3") 'adb-split-right)
   #+end_src
